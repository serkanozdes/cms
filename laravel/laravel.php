 ...
 * configuration class, and the class auto-loader.
 * Once this file
...
/**
 * Create the exception logging function.
 * All of the error logging
...
$handler = function  ($exception) use( $logger)
{
    ...
    if (Config::$items['error']['detail']) {
        echo "<html><h2>Unhandled Exception</h2>
			  ...
			  <h3>Location:</h3>
			  <pre>" .
                 $exception->getFile() . " on line " . $exception->getLine() . "</pre>
			  <h3>Stack Trace:</h3>
			  <pre>" .
                 $exception->getTraceAsString() . "</pre></html>";
} else {
 ...
 * All PHP errors will fall into this
...
/**
 * Load the session and session manager instance.
 * The session
 ...
 * unexpectedly included in Input::all() or Input::get().
 * Leaving it
...
/**
 * Route the request to the proper route in the application.
 * If a
 ...
 * storage.
 * The session cookie will also be written and if the
