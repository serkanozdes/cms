...
class Payload
{
    ...
    public function load ($id)
    {
        if (! is_null($id))
            ...
            // If the session doesn't exist or is invalid, we will create a new
        // session
            // array and mark the session as being non-existent. Some drivers,
        // such as
            // the database driver, need to know whether the session exists in
        // storage
            // so they can know whether to "insert" or "update" the session.
        if (is_null($this->session) or $this->invalid()) {
            $this->exists = false;
            
            $this->session = array('id' => Str::random(40), 
                    'data' => array(':new:' => array(), ':old:' => array()));
        }
        
        // A CSRF token is stored in every session. The token is used by the
        // Form
        // class and the "csrf" filter to protect the application from
        // cross-site
    ...
    {
        $this->session['data'] = array(':new:' => array(), ':old:' => array());
    }
    ...
    public function save ()
    {
        ...
        // they
        // must clean up expired sessions manually. If the driver is a sweeper,
        // we
        // need to determine if garbage collection should be run for the
        // request.
        ...
        if ($this->driver instanceof Sweeper and
                 (mt_rand(1, $sweepage[1]) <= $sweepage[0])) {
             ...
             *
             * Session flash data is only available during the request in which
             * it
             * was flashed, and the request after that. To "age" the data, we
             * will
            ...
            protected function cookie ()
            {
                ...
                Cookie::put($cookie, $this->session['id'], $minutes, $path, 
                        $domain, $secure);
            }
