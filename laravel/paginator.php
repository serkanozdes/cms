...
class Paginator
{
    ...
    public static function page ($total, $per_page)
    {
        ...
        // The page will be validated and adjusted if it is less than one or
        // greater
        // than the last page. For example, if the current page is not an
        // integer or
        // less than one, one will be returned. If the current page is greater
        // than
     ...
     *
     * Typically, an intelligent, "sliding" window of links will be rendered
     * based
     * on the total number of pages, the current page, and the number of
     * adjacent
    ...
    public function links ($adjacent = 3)
    {
        if ($this->last <= 1)
            ...
            // The hard-coded seven is to account for all of the constant
        // elements in a
            // sliding range, such as the current page, the two ellipses, and
        // the two
            // beginning and ending pages.
            //
            // If there are not enough pages to make the creation of a slider
        // possible
            // based on the adjacent pages, we will simply display all of the
        // pages.
    ...
    public function slider ($adjacent = 3)
    {
        ...
        elseif ($this->page >= $this->last - $window) {
            return $this->beginning() . ' ' .
                     $this->range($this->last - $window - 2, $this->last);
                }
                
                // Example: 1 2 ... 23 24 25 [26] 27 28 29 ... 51 52
                $content = $this->range($this->page - $adjacent, 
                        $this->page + $adjacent);
                
                return $this->beginning() . ' ' . $content . ' ' .
                         $this->ending();
                    }
                     ...
                     *
                     * // Create the "previous" pagination element with custom
                     * text
                    ...
                    public function previous ($text = null)
                    {
                        ...
                        return $this->element(__FUNCTION__, $this->page - 1, 
                                $text, $disabled);
                    }
                    ...
                    /**
                     * Create a chronological pagination element, such as a
                     * "previous" or "next" link.
                     *
                     * @param $element string           
                     * @param $page int           
                     * @param $text string           
                     * @param $disabled Closure           
                     * @return string
                     */
                    protected function element ($element, $page, $text, 
                            $disabled)
                    {
                        $class = "{$element}_page";
                        
                        if (is_null($text))
                            $text = Lang::line("pagination.{$element}")->get(
                                    $this->language);
                            
                            // Each consumer of this method provides a
                        // "disabled" Closure which can
                            // be used to determine if the element should be a
                        // span element or an
                            // actual link. For example, if the current page is
                        // the first page,
                            // the "first" element should be a span instead of a
                        // link.
                        if ($disabled($this->page, $this->last)) {
                            return HTML::span($text, 
                                    array('class' => "{$class} disabled"));
                        } else {
                    ...
                    protected function ending ()
                    {
                        return $this->dots . ' ' .
                             ...
                             *
                             * For the current page, an HTML span element will
                             * be generated instead of a link.
                             *
                             * @param $start int           
                             * @param $end int           
                             * @return string
                            ...
                            protected function range ($start, $end)
                            {
                                ...
                                // To generate the range of page links, we will
                                // iterate through each page
                                // and, if the current page matches the page, we
                                // will generate a span,
                                // otherwise we will generate a link for the
                                // page. The span elements
                                // will be assigned the "current" CSS class for
                                // convenient styling.
                                for ($page = $start; $page <= $end; $page ++) {
                                    if ($this->page == $page) {
                                        $pages[] = HTML::span($page, 
                                                array('class' => 'current'));
                                    } else {
                                        $pages[] = $this->link($page, $page, 
                                                null);
                                    }
                                }
                            ...
                            {
                                $query = '?page=' . $page .
                                         $this->appendage($this->appends);
                                        
                                        return HTML::link(URI::current() .
                                                 $query, $text, compact('class'), 
                                                        Request::secure());
                                    }

                                    /**
                                     * Create the "appendage" that should be
                                     * attached to every pagination link.
                                     *
                                     * The developer may assign an array of
                                     * values that will be converted to a
                                     * query string and attached to every
                                     * pagination link. This allows simple
                                     * implementation of sorting or other things
                                    ...
                                    protected function appendage ($appends)
                                    {
                                        ...
                                        return $this->appendage = (count(
                                                $appends) > 0) ? '&' .
                                                 http_build_query($appends) : '';
                                            }

                                            /**
                                             * Set the items that should be
                                             * appended to the link query
                                             * strings.
                                             *
                                             * @param $values array           
                                             * @return Paginator
                                            ...
                                            /**
                                             * Set the language that should be
                                             * used when creating the pagination
                                             * links.
                                             *
