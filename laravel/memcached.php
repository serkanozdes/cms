...
class Memcached
{
     ...
     * $memcache = Memcached::connect(array('host' => '127.0.0.1', 'port' =>
     * 11211));
     * </code>
     *
     * @param $servers array           
    ...
    public static function connect ($servers)
    {
        ...
        foreach ($servers as $server) {
            $memcache->addServer($server['host'], $server['port'], true, 
                    $server['weight']);
        }
    ...
    {
        return call_user_func_array(array(static::instance(), $method), 
                $parameters);
    }
