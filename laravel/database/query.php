...
class Query
{
     ...
     * @param $grammar Grammars\Grammar           
     * @param $table string           
     * @return void
     */
    public function __construct (Connection $connection, 
            Grammars\Grammar $grammar, $table)
    {
        $this->from = $table;
    ...
    {
        $this->joins[] = compact('type', 'table', 'column1', 'operator', 
                'column2');
    ...
    {
        $this->wheres[] = array('type' => 'where_raw', 
                'connector' => $connector, 'sql' => $where);
     ...
     *
     * Dynamic queries are caught by the __call magic method and are parsed
     * here.
    ...
    private function dynamic_where ($method, $parameters)
    {
        ...
        $segments = preg_split('/(_and_|_or_)/i', $finder, - 1, 
    ...
    public function only ($column)
    {
        ...
        return $this->connection->only($this->grammar->select($this), 
                $this->bindings);
    }
     ...
     *
     * If a single column is selected from the database, only the value of that
    ...
    public function get ($columns = array('*'))
    {
        ...
        $results = $this->connection->query($this->grammar->select($this), 
                $this->bindings);
    ...
    private function aggregate ($aggregator, $column)
    {
        ...
        $result = $this->connection->only($this->grammar->select($this), 
                $this->bindings);
    ...
    public function paginate ($per_page = 20, $columns = array('*'))
    {
        ...
        return Paginator::make($this->for_page($page, $per_page)->get($columns), 
                $total, $per_page);
    }
    ...
    public function insert ($values)
    {
        ...
        return $this->connection->query($this->grammar->insert($this, $values), 
                $bindings);
    }
    ...
    {
        $this->connection->query($this->grammar->insert($this, $values), 
                array_values($values));
    ...
    {
        $value = Manager::raw($this->grammar->wrap($column) . $operator .
                 $amount);
    ...
    public function update ($values)
    {
        ...
        return $this->connection->query($this->grammar->update($this, $values), 
                $bindings);
    }
    ...
    public function delete ($id = null)
    {
        ...
        return $this->connection->query($this->grammar->delete($this), 
                $this->bindings);
    }
    ...
    public function __call ($method, $parameters)
    {
        ...
        throw new \BadMethodCallException(
                "Method [$method] is not defined on the Query class.");
    }
