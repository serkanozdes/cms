...
abstract class Model
{
    ...
    public static function query ($class)
    {
        ...
        $model->query = DB::connection(static::$connection)->table(
                static::table($class));
    ...
    private function _paginate ($per_page = null)
    {
        ...
        // The number of models to show per page may be specified as a static
        // property
        // on the model. The models shown per page may also be overriden for the
        // model
        // by passing the number into this method. If the models to show per
        // page is
        // not available via either of these avenues, a default number will be
        // shown.
        if (is_null($per_page)) {
        ...
        return Paginator::make(
                $this->for_page(Paginator::page($total, $per_page), $per_page)->get(), 
                $total, $per_page);
    }
     ...
     *
     * The default foreign key for has one and has many relationships is the
     * name
    ...
    {
        $this->relating_key = (is_null($foreign_key)) ? strtolower(
                static::model_name($this)) . '_id' : $foreign_key;
    ...
    public function belongs_to ($model, $foreign_key = null)
    {
        ...
        return static::query($model)->where('id', '=', 
                $this->attributes[$this->relating_key]);
    }

    /**
     * Retrieve the query for a *:* relationship.
     *
     * The default foreign key for many-to-many relations is the name of the
     * model
     * with an appended _id. This is the same convention as has_one and
     * has_many.
     *
     * @param $model string           
     * @param $table string           
     * @param $foreign_key string           
     * @param $associated_key string           
     * @return mixed
     */
    public function has_and_belongs_to_many ($model, $table = null, 
            $foreign_key = null, $associated_key = null)
    {
        $this->relating = __FUNCTION__;
        
        $this->relating_table = (is_null($table)) ? $this->intermediate_table(
                $model) : $table;
        ...
        $associated_key = (is_null($associated_key)) ? strtolower(
                static::model_name($model)) . '_id' : $associated_key;
        
        return static::query($model)->select(array(static::table($model) . '.*'))
            ->join($this->relating_table, static::table($model) . '.id', '=', 
                $this->relating_table . '.' . $associated_key)
    ...
    {
        $models = array(Inflector::plural(static::model_name($model)), 
                Inflector::plural(static::model_name($this)));
    ...
    public function save ()
    {
        ...
        // Since the model was instantiated using "new", a query instance has
        // not been set.
        // Only models being used for querying have their query instances set by
        // default.
        $this->query = DB::connection(static::$connection)->table(
                static::table($model));
        ...
        if ($this->exists) {
            $success = ($this->query->where_id($this->attributes['id'])->update(
                    $this->dirty) === 1);
        } else {
            $success = is_numeric(
                    $this->attributes['id'] = $this->query->insert_get_id(
                            $this->attributes, static::$sequence));
        }
    ...
    {
        // If the delete method is being called on an existing model, we only
        // want to delete
        // that model. If it is being called from an Eloquent query model, it is
        // probably
        // the developer's intention to delete more than one model, so we will
        // pass the
        ...
        return DB::connection(static::$connection)->table($table)->delete(
                $this->id);
    }
    ...
    public function __get ($key)
    {
        ...
        }        // Is the requested item a model relationship that has already been
        // loaded?
        // All of the loaded relationships are stored in the "ignore" array.
        elseif (array_key_exists($key, $this->ignore)) {
            return $this->ignore[$key];
        }        // Is the requested item a model relationship? If it is, we will
        // dynamically
        // load it and return the results of the relationship query.
        elseif (method_exists($this, $key)) {
            $query = $this->$key();
            
            return $this->ignore[$key] = (in_array($this->relating, 
                    array('has_one', 'belongs_to'))) ? $query->first() : $query->get();
        }
    }
    ...
    {
        return (array_key_exists($key, $this->attributes) or
                 array_key_exists($key, $this->ignore));
            }
            ...
            {
                unset($this->attributes[$key], $this->ignore[$key], 
                        $this->dirty[$key]);
            }
            ...
            {
                // To allow the "with", "get", "first", and "paginate" methods
                // to be called both
                // staticly and on an instance, we need to have private,
                ...
                // All of the aggregate and persistance functions can be passed
                // directly to the query
                // instance. For these functions, we can simply return the
                // response of the query.
                if (in_array($method, 
                        array('insert', 'update', 'increment', 'decrement', 
                                'abs', 'count', 'sum', 'min', 'max', 'avg'))) {
                    return call_user_func_array(array($this->query, $method), 
                            $parameters);
                }
                
                // Pass the method to the query instance. This allows the
                // chaining of methods
                // from the query builder, providing the same convenient query
                // API as the
            ...
            {
                // Just pass the method to a model instance and let the __call
                // method take care of it.
                return call_user_func_array(
                        array(static::query(get_called_class()), $method), 
                        $parameters);
            }
