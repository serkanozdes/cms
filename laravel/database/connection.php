...
class Connection
{
    ...
    protected function grammar ()
    {
        if (isset($this->grammar))
            ...
            // We allow the developer to hard-code a grammar for the connection.
        // This really
            // has no use yet; however, if database systems that can use
        // multiple grammars
     ...
     * $user = DB::connection()->first('select * from users where id = ?',
     * array($id));
     * </code>
     *
     * @param $sql string           
     ...
     *
     * // Execute a query with bound parameters
     * $user = DB::connection()->query('select * from users where id = ?',
     * array($id));
     * </code>
     *
     * @param $sql string           
    ...
    protected function transform ($sql, $bindings)
    {
        if (strpos($sql, '(...)') !== false) {
            for ($i = 0; $i < count($bindings); $i ++) {
                // If the binding is an array, we can assume it is being used to
                // fill
                // a "where in" condition, so we will replace the next
                // place-holder
                // in the query with the correct number of parameters based on
                // the
                // number of elements in this binding.
                if (is_array($bindings[$i])) {
                    $parameters = implode(', ', 
                            array_fill(0, count($bindings[$i]), '?'));
                    
                    $sql = preg_replace('~\(\.\.\.\)~', "({$parameters})", $sql, 
                            1);
                }
            }
        }
