...
class Redirect extends Response
{
    ...
    {
        return static::make('', $status)->header('Location', 
                URL::to($url, $https));
    }
     ...
     *
     * This is useful for passing status messages or other temporary data to the
     * next request.
     *
     * <code>
     ...
     * return Redirect::to_login()->with_input('only', array('email',
     * 'username'));
     *
     * // Redirect and flash all but a few of the input items
     * return Redirect::to_login()->with_input('except', array('password',
     * 'ssn'));
     * </code>
     *
     * @param $filter string           
     ...
     * </code>
     *
     * @param $container Validator|Messages           
     * @return Redirect
     */
    ...
    {
        // Extract the parameters that should be placed in the URL. These
        // parameters
        // are used to fill all of the wildcard slots in the route URI
        // definition.
        ...
        if (strpos($method, 'to_secure_') === 0) {
            return static::to(
                    URL::to_route(substr($method, 10), $wildcards, true), 
                    $status);
        }
        
        if (strpos($method, 'to_') === 0) {
            return static::to(URL::to_route(substr($method, 3), $wildcards), 
                    $status);
        }
        
        throw new \BadMethodCallException(
                "Method [$method] is not defined on the Redirect class.");
    }
