...
class View
{
    ...
    public function __construct ($view, $data = array())
    {
        ...
        if (Config::$items['session']['driver'] !== '' and
                 IoC::core('session')->started()) {
                    $this->data['errors'] = IoC::core('session')->get('errors', 
                            function  ()
                            {
                                return new Messages();
                            });
             ...
             * Create a new view instance.
             *
             * The name of the view given to this method should correspond to a
             * view
             * within your application views directory. Dots or slashes may used
             * to
            ...
            public static function of ($name, $data = array())
            {
                ...
                throw new \OutOfBoundsException(
                        "Named view [$name] is not defined.");
            }

            /**
             * Find the key for a view by name.
             *
             * The view "key" is the string that should be passed into the
             * "make" method and
             * should correspond with the location of the view within the
             * application views
            ...
            protected static function name ($name)
            {
                ...
                foreach (static::$composers as $key => $value) {
                        ...
                        if (isset(static::$composers[$view->view])) {
                            foreach ((array) static::$composers[$view->view] as $key => $value) {
                                if ($value instanceof Closure)
                                    return call_user_func($value, $view);
                            }
                        }
                    }
                     ...
                     *
                     * For better testing flexiblity, we load the composers from
                     * the IoC container.
                     *
                     * @return void
                     */
                    ...
                    public function render ()
                    {
                        ...
                        // All nested views and responses are evaluated before
                        // the main view.
                        // This allows the assets used by the nested views to be
                        // added to the
                        // asset container before the main view is evaluated and
                        // dumps the
                        // links to the assets.
                        foreach ($this->data as &$data) {
                            if ($data instanceof View or
                                ...
                                // If the view is Bladed, we need to check the
                                // view for modifications
                                // and get the path to the compiled view file.
                            ...
                            /**
                             * Compile the Bladed view and return the path to
                             * the compiled view.
                             *
                             * @return string
                             */
                            ...
                            {
                                // For simplicity, compiled views are stored in
                                // a single directory by
                                // the MD5 hash of their name. This allows us to
                                // avoid recreating the
                                // entire view directory structure within the
                                // compiled directory.
                                $compiled = STORAGE_PATH . 'views/' .
                                         md5($this->view);
                                        
                                        // The view will only be re-compiled if
                                        // the view has been modified
                                        // since the last compiled version of
                                        // the view was created or no
                                        // compiled view exists. Otherwise, the
                                        // path will be returned
                                        // without re-compiling.
                                        if (! file_exists($compiled) or
                                                 (filemtime($this->path) >
                                                 filemtime($compiled))) {
                                     ...
                                     * $view = View::make('foo')->nest('footer',
                                     * 'partials.footer');
                                     *
                                     * // Equivalent functionality using the
                                     * "with" method
                                     * $view = View::make('foo')->with('footer',
                                     * View::make('partials.footer'));
                                     *
                                     * // Bind a view instance with data
                                     * $view = View::make('foo')->nest('footer',
                                     * 'partials.footer', array('name' =>
                                     * 'Taylor'));
                                     * </code>
                                     *
                                     * @param $key string           
                                     ...
                                     *
                                     * Bound data will be available to the view
                                     * as variables.
                                     *
                                     * @param $key string           
                                     * @param $value mixed           
                                    ...
                                    /**
                                     * Magic Method for getting items from the
                                     * view data.
                                     */
                                    ...
                                    /**
                                     * Magic Method for setting items in the
                                     * view data.
                                     */
                                    ...
                                    /**
                                     * Magic Method for determining if an item
                                     * is in the view data.
                                     */
                                    ...
                                    /**
                                     * Magic Method for removing an item from
                                     * the view data.
                                     */
                                    ...
                                    /**
                                     * Get the evaluated string content of the
                                     * view.
                                     *
                                     * @return string
                                     */
                                    ...
                                    /**
                                     * Magic Method for handling the dynamic
                                     * creation of named views.
                                     *
                                     * <code>
                                     ...
                                     * $view = View::of_layout();
                                     *
                                     * // Create an instance of a named view
                                     * with data
                                     * $view = View::of_layout(array('name' =>
                                     * 'Taylor'));
                                     * </code>
                                     */
                                            ...
                                            $parameters)
                                    {
                                        ...
                                        throw new \BadMethodCallException(
                                                "Method [$method] is not defined on the View class.");
                                    }
