...
class Autoloader
{
    ...
    public static function load ($class)
    {
        if (isset(Config::$items['application']['aliases'][$class])) {
            return class_alias(Config::$items['application']['aliases'][$class], 
                    $class);
        }
    ...
    {
        // First we will look for the class in the hard-coded class mappings,
        // since
        // this is the fastest way to resolve a class name to its associated
        // file.
        ...
        // If the library has been registered as a PSR-0 compliant library, we
        // will
        // load the library according to the PSR-0 naming standards, which state
        // that
        // namespaces and underscores indicate the directory hierarchy of the
        // class.
        if (in_array(static::library($class), static::$libraries)) {
            return LIBRARY_PATH . str_replace(array('\\', '_'), '/', $class) .
                     EXT;
                }
                
                // Next we will search through the common Laravel paths for the
                // class file.
                // The Laravel libraries and models directories will be searched
                // according
                ...
                // Since not all controllers will be resolved by the controller
                // resolver,
                // we will do a quick check in the controller directory for the
                // class.
                // For instance, since base controllers would not be resolved by
                // the
             ...
             *
             * The library name is essentially the namespace, or the string that
             * preceeds
             * the first PSR-0 separator. PSR-0 states that namespaces or
             * undescores may
             * be used to indicate the directory structure in which the file
             * resides.
             *
             * @param $class string           
             * @return string
            ...
            /**
             * Translate a given controller class name into the corresponding
             * file name.
             *
             * The controller suffix will be removed, and the underscores will
             * be translated
             * into directory slashes. Of course, the entire class name will be
             * converted to
             * lower case as well.
             *
             * <code>
            ...
            {
                $controller = str_replace(array('_', '_Controller'), 
                        array('/', ''), $class);
             ...
             *
             * The mappings will be used to resolve file paths from class names
             * when
             * a class is lazy loaded through the Autoloader, providing a faster
             * way
             ...
             *
             * The library names given to this method should match directories
             * within
             * the application libraries directory. This method provides an easy
             * way
            ...
            {
                static::$libraries = array_merge(static::$libraries, 
                        (array) $libraries);
            }
