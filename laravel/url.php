...
class URL
{
     ...
     *
     * If the application URL is explicitly defined in the application
     * configuration
     * file, that URL will be returned. Otherwise, the URL will be guessed based
     * on
    ...
    {
        if (($base = Config::$items['application']['url']) !== '')
            return $base;
        
        if (isset($_SERVER['HTTP_HOST'])) {
            $protocol = (Request::secure()) ? 'https://' : 'http://';
            
            // By removing the basename of the script, we should be left with
            // the path
            // in which the framework is installed. For example, if the
            // framework is
            // installed to http://localhost/laravel/public, the path we'll get
            // from
            // this statement will be "/laravel/public".
            $path = str_replace(basename($_SERVER['SCRIPT_NAME']), '', 
                    $_SERVER['SCRIPT_NAME']);
    ...
    public static function to ($url = '', $https = false)
    {
        ...
        // Since SSL is often not used while developing the application, we
        // allow the
        // developer to disable SSL on all framework generated links to make it
        // more
    ...
    public static function to_asset ($url, $https = null)
    {
        ...
        // Since assets are not served by Laravel, we do not need to come
        // through
    ...
    public static function to_route ($name, $parameters = array(), $https = false)
    {
        if (! is_null($route = IoC::core('routing.router')->find($name))) {
            ...
            return static::to(
                    str_replace(array('/(:any?)', '/(:num?)'), '', $uri), $https);
        }
        
        throw new \OutOfBoundsException(
                "Error creating URL for undefined route [$name].");
    }
    ...
    public static function __callStatic ($method, $parameters)
    {
        ...
        throw new \BadMethodCallException(
                "Method [$method] is not defined on the URL class.");
    }
