...
class Cookie
{
    ...
    public static function get ($name, $default = null)
    {
        ...
        if (! is_null($value)) {
            ...
            // The hash serves to verify that the contents of the cookie have
            // not
            // been modified by the user. We can verify the integrity of the
            // cookie
            // by extracting the value and re-hashing it, then comparing that
            // hash
     ...
     * The cookie will last for one year.
     *
     * @param $name string           
     * @param $value string           
     * @param $path string           
     * @param $domain string           
     * @param $secure bool           
     * @param $http_only bool           
     * @return bool
     */
    public static function forever ($name, $value, $path = '/', $domain = null, 
     ...
     * @return bool
     */
    public static function put ($name, $value, $minutes = 0, $path = '/', 
            $domain = null, $secure = false, $http_only = false)
    {
        if (headers_sent())
            return false;
     ...
     * been modified by the user, since they serve as a fingerprint of the
     * cookie
     * contents. The application key is used to salt the salts.
     *
     * When the cookie is read using the "get" method, the value will be
     * extracted
     * from the cookie and hashed, if the hash in the cookie and the hashed
     * value
