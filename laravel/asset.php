...
class Asset
{
    ...
    {
        return call_user_func_array(array(static::container(), $method), 
                $parameters);
    }
...
class Asset_Container
{
     ...
     * asset being registered (CSS or JavaScript). If you are using a
     * non-standard
    ...
    public function add ($name, $source, $dependencies = array(), $attributes = array())
    {
        ...
        return call_user_func(array($this, $type), $name, $source, 
     ...
     * @return Asset_Container
     */
    public function style ($name, $source, $dependencies = array(), 
            $attributes = array())
    {
     ...
     * @return Asset_Container
     */
    public function script ($name, $source, $dependencies = array(), 
            $attributes = array())
    {
    ...
    protected function arrange ($assets)
    {
        ...
        while (count($assets) > 0) {
            foreach ($assets as $asset => $value) {
                $this->evaluate_asset($asset, $value, $original, $sorted, 
                        $assets);
            }
        }
     ...
     * @return void
     */
    protected function evaluate_asset ($asset, $value, $original, &$sorted, 
            &$assets)
    {
        ...
        // the asset's dependencies and determine if they have already been
        // sorted.
        if (count($assets[$asset]['dependencies']) == 0) {
            ...
            foreach ($assets[$asset]['dependencies'] as $key => $dependency) {
                if (! $this->dependency_is_valid($asset, $dependency, $original, 
                        $assets)) {
                ...
                // we can not
                // remove it from this asset's array of dependencies. We'll try
                // again on
     ...
     * @return bool
     */
    protected function dependency_is_valid ($asset, $dependency, $original, 
            $assets)
    {
        ...
        } elseif (isset($assets[$dependency]) and
                 in_array($asset, $assets[$dependency]['dependencies'])) {
                    throw new \LogicException(
                            "Assets [$asset] and [$dependency] have a circular dependency.");
                }
            }
