...
class Loader
{
    ...
    public function load ($uri)
    {
        ...
        return array_merge($this->nested($segments), 
                require $this->base . 'routes' . EXT);
    }
    ...
    protected function nested ($segments)
    {
        foreach (array_reverse($segments, true) as $key => $value) {
            $path = $this->nest .
                     implode('/', array_slice($segments, 0, $key + 1)) . EXT;
             ...
             *
             * The entire routes directory will be searched recursively to
             * gather
             * every route for the application. Of course, the routes in the
             * root
            ...
            {
                if (! is_null($this->everything))
                    return $this->everything;
                
                $routes = array();
                
                // First, we'll grab the base routes from the application
                // directory.
                // Once we have these, we'll merge all of the nested routes in
                // the
                ...
                $iterator = new Iterator(new DirectoryIterator($this->nest), 
                        Iterator::SELF_FIRST);
                
                foreach ($iterator as $file) {
                    // Since some Laravel developers may place HTML files in the
                    // route
                    // directories, we will check for the PHP extension before
                    // merging
                    // the file. Typically, the HTML files are present in
                    // installations
                    // that are not using mod_rewrite and the public directory.
                    if (filetype($file) === 'file' and
                             strpos($file, EXT) !== false) {
