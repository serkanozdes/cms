...
class Filter
{
    ...
    public static function run ($filters, $pass = array(), $override = false)
    {
        foreach (static::parse($filters) as $filter) {
            ...
            $response = call_user_func_array(static::$filters[$filter], 
                    $parameters);
...
class Filter_Collection
{
     ...
     *
     * Methods may be included / excluded using the "only" and "except" methods
     * on the
     * filter collection. Also, the "on" method may be used to set certain
     * filters to
    ...
    public function applies ($method)
    {
        ...
        if (count($this->methods) > 0 and
                 ! in_array(strtolower(Request::method()), $this->methods)) {
                    return false;
                }
             ...
             *
             * When methods are excluded, the collection's filters will be run
             * for each
             * controller method except those explicitly specified via this
             * method.
             *
             * <code>
             ...
             *
             * This method is the inverse of the "except" methods. The methods
             * specified
             * via this method are the only controller methods on which the
             * collection's
             * filters will be run.
             *
             * <code>
             ...
             *
             * Since some filters, such as the CSRF filter, only make sense in a
             * POST
             * request context, this method allows you to limit which HTTP
             * methods
             ...
             *
             * // Specify that a filter applies for multiple HTTP request
             * methods
