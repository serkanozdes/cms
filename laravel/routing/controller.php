...
abstract class Controller
{
     ...
     *
     * The controller destination should follow a {controller}@{method}
     * convention.
     * Nested controllers may be delegated to using dot syntax.
     *
     * For example, a destination of "user.profile@show" would call the
     * User_Profile
    ...
    public static function call ($destination, $parameters = array())
    {
        if (strpos($destination, '@') === false) {
            throw new \InvalidArgumentException(
                    "Route delegate [{$destination}] has an invalid format.");
        }
    ...
    public static function resolve ($controller)
    {
        ...
        // resolve
            // it out of the container. Using constructor injection on
        // controllers
        ...
        $controller = str_replace(' ', '_', 
                ucwords(str_replace('.', ' ', $controller))) . '_Controller';
    ...
    public function __get ($key)
    {
        ...
        throw new \OutOfBoundsException(
                "Attempting to access undefined property [$key] on controller.");
    }
