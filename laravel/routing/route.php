...
class Route
{
    ...
    public function __construct ($key, $callback, $parameters = array())
    {
        ...
        if (strpos($key, ', ') === false) {
            ...
            $this->uris = array_map(array($this, 'extract'), 
                    explode(', ', $key));
        }
        
        if (! $this->callable($callback)) {
            throw new \InvalidArgumentException(
                    'Invalid route defined for URI [' . $this->key . ']');
        }
    }
    ...
    {
        return $callback instanceof Closure or is_array($callback) or
                 is_string($callback);
            }
            ...
            {
                // Since "before" filters can halt the request cycle, we will
                // return
                // any response from the before filters. Allowing filters to
                // halt the
                // request cycle makes tasks like authorization convenient.
                //
                // The route is responsible for running the global filters, and
                // any
                // filters defined on the route itself. Since all incoming
                // requests
                // come through a route (either defined or ad-hoc), it makes
                // sense
                ...
                if (is_null($response) and
                         ! is_null($response = $this->response())) {
                        ...
                        // Stringify the response. We need to force the response
                        // to be
                        // stringed before closing the session, since the
                        // developer may
                        // be using the session within their views, so we cannot
                        // age
                        // the session data until the view is rendered.
                        $response->content = $response->render();
                        
                        $filters = array_merge($this->filters('after'), 
                                array('after'));
                    ...
                    /**
                     * Call the closure defined for the route, or get the route
                     * delegator.
                     *
                     * Note that this method differs from the "call" method in
                     * that it does
                     * not resolve the controller or prepare the response.
                     * Delegating to
                    ...
                    {
                        // If the route callback is an instance of a Closure, we
                        // can call the
                        // route function directly. There are no before or after
                        // filters to
                        // parse out of the route.
                        if ($this->callback instanceof Closure) {
                            return call_user_func_array($this->callback, 
                                    $this->parameters);
                        }                        // If the route is an array, we will return the first
                        // value with a
                        // key of "uses", or the first instance of a Closure. If
                        // the value
                        // is a string, the route is delegating the
                        // responsibility for
                        // for handling the request to a controller.
                        elseif (is_array($this->callback)) {
                            $callback = Arr::first($this->callback, 
                                    function  ($key, $value)
                                    {
                                        return $key == 'uses' or
                    ...
                    public function filters ($name)
                    {
                        if (is_array($this->callback) and
                                 isset($this->callback[$name])) {
                                    $filters = $this->callback[$name];
                                    
                                    return (is_string($filters)) ? explode('|', 
                            ...
                            {
                                return is_array($this->callback) and
                                         Arr::get($this->callback, 'name') ===
                                         $name;
                            }
                            ...
                            /**
                             * Magic Method to handle dynamic method calls to
                             * determine the name of the route.
                             */
                            public function __call ($method, $parameters)
                            {
                                ...
                                throw new \BadMethodCallException(
                                        "Call to undefined method [$method] on Route class.");
                            }
