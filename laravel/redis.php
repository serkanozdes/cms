...
class Redis
{
        ...
        if (! isset(static::$databases[$name])) {
            if (is_null($config = Config::get("database.redis.{$name}"))) {
                throw new \DomainException(
                        "Redis database [$name] is not defined.");
            }
            
            static::$databases[$name] = new static($config['host'], 
                    $config['port']);
        }
        ...
        switch (substr($response, 0, 1)) {
            case '-':
                throw new \RuntimeException(
                        'Redis error: ' . substr(trim($response), 4));
            ...
            default:
                throw new \UnexpectedValueException(
                        "Unknown Redis response: " . substr($response, 0, 1));
        }
    }
    ...
    protected function connect ()
    {
        ...
        $this->connection = @fsockopen($this->host, $this->port, $error, 
                $message);
        
        if ($this->connection === false) {
            throw new \RuntimeException(
                    "Error making Redis connection: {$error} - {$message}");
        }
    ...
    protected function bulk ($head)
    {
        ...
        do {
            // Calculate and read the appropriate bytes off of the Redis
            // response.
